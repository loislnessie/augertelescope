...
const MEASUREMENT_TIME = 10000 * 6 // 10s
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            forEachCard(card => {
                ...
                let diff = 100 * Math.abs((eventsPerSecond - OPTIMAL_EVENTS_PER_SECOND) / ((eventsPerSecond + OPTIMAL_EVENTS_PER_SECOND) / 2))
                if(eventsPerSecond > OPTIMAL_EVENTS_PER_SECOND) diff *= -1
    
                console.log(card.name + '(' + card.threshold + '): ' + count + ' events in ' + MEASUREMENT_TIME / 1000 + 's = ' + eventsPerSecond + ' events per s = ' + diff + '%')
    
    
                if(Math.abs(diff) >= MAX_PERCENTUAL_DIFFERENCE) { //wenn die differenz größer als 10% ist
            ...
            console.log('\n')
            setTimeout(startMeasurement, 1000)
        }, 1000)
        // startMeasurement()
    }, MEASUREMENT_TIME)
}
